#include "RecoVertex/PrimaryVertexProducer/interface/DAClusterizerInZ_vectCUDA.h"
...
DAClusterizerInZ_vectCUDA::DAClusterizerInZ_vectCUDA(const edm::ParameterSet& conf) {
  ...
  std::cout << "DAClusterizerInZ_vectCUDA: uniquetrkminp = " << uniquetrkminp_ << std::endl;
  ...
  std::cout << "DAClusterizerInZ_vectCUDA: zrange = " << sel_zrange_ << std::endl;
  ...
  if (convergence_mode_ > 1) {
    edm::LogWarning("DAClusterizerinZ_vect")
        << "DAClusterizerInZ_vectCUDA: invalid convergence_mode " << convergence_mode_ << "  reset to default " << 0;
    convergence_mode_ = 0;
  }
  ...
  if (Tmin == 0) {
    betamax_ = 1.0;
    edm::LogWarning("DAClusterizerinZ_vect")
        << "DAClusterizerInZ_vectCUDA: invalid Tmin " << Tmin << "  reset to default " << 1. / betamax_;
  } else {
    betamax_ = 1. / Tmin;
  }
  ...
  if ((Tpurge > Tmin) || (Tpurge == 0)) {
    edm::LogWarning("DAClusterizerinZ_vect")
        << "DAClusterizerInZ_vectCUDA: invalid Tpurge " << Tpurge << "  set to " << Tmin;
    Tpurge = Tmin;
  }
  betapurge_ = 1. / Tpurge;
  ...
  if ((Tstop > Tpurge) || (Tstop == 0)) {
    edm::LogWarning("DAClusterizerinZ_vect")
        << "DAClusterizerInZ_vectCUDA: invalid Tstop " << Tstop << "  set to  " << max(1., Tpurge);
    Tstop = max(1., Tpurge);
  }
  betastop_ = 1. / Tstop;
}
...
void DAClusterizerInZ_vectCUDA::verify(const vertex_t& v, const track_t& tks, unsigned int nv, unsigned int nt) const {
  if (!(nv == 999999)) {
    assert(nv == v.getSize());
  } else {
    nv = v.getSize();
...
track_t DAClusterizerInZ_vectCUDA::fill(const vector<reco::TransientTrack>& tracks) const {
...
void DAClusterizerInZ_vectCUDA::set_vtx_range(double beta, track_t& gtracks, vertex_t& gvertices) const {
...
void DAClusterizerInZ_vectCUDA::clear_vtx_range(track_t& gtracks, vertex_t& gvertices) const {
  const unsigned int nt = gtracks.getSize();
  const unsigned int nv = gvertices.getSize();
  for (auto itrack = 0U; itrack < nt; ++itrack) {
...
double DAClusterizerInZ_vectCUDA::update(
...
unsigned int DAClusterizerInZ_vectCUDA::thermalize(
...
#ifdef DEBUG
  if (DEBUGLEVEL > 0) {
    std::cout << "DAClusterizerInZ_vectCUDA.thermalize niter = " << niter << " at T = " << 1 / beta
...
bool DAClusterizerInZ_vectCUDA::merge(vertex_t& y, track_t& tks, double& beta) const {
...
bool DAClusterizerInZ_vectCUDA::purge(vertex_t& y, track_t& tks, double& rho0, const double beta) const {
...
double DAClusterizerInZ_vectCUDA::beta0(double betamax, track_t const& tks, vertex_t const& y) const {
...
#ifdef DEBUG
  if (DEBUGLEVEL > 0) {
    std::cout << "DAClusterizerInZ_vectCUDA.beta0:   Tc = " << T0 << std::endl;
    int coolingsteps = 1 - int(std::log(T0 * betamax) / std::log(coolingFactor_));
    std::cout << "DAClusterizerInZ_vectCUDA.beta0:   nstep = " << coolingsteps << std::endl;
  }
#endif
...
bool DAClusterizerInZ_vectCUDA::split(const double beta, track_t& tks, vertex_t& y, double threshold) const {
...
vector<TransientVertex> DAClusterizerInZ_vectCUDA::vertices(const vector<reco::TransientTrack>& tracks) const {
  ...
  if (DEBUGLEVEL > 0) {
    std::cout << "DAClusterizerInZ_vectCUDA::vertices :"
...
vector<vector<reco::TransientTrack> > DAClusterizerInZ_vectCUDA::clusterize(
  ...
  if (DEBUGLEVEL > 0) {
    std::cout << "###################################################" << endl;
    std::cout << "# vectorized DAClusterizerInZ_vectCUDA::clusterize   nt=" << tracks.size() << endl;
    std::cout << "# DAClusterizerInZ_vectCUDA::clusterize   pv.size=" << pv.size() << endl;
    std::cout << "###################################################" << endl;
  }
#endif
...
void DAClusterizerInZ_vectCUDA::dump(
...
void DAClusterizerInZ_vectCUDA::fillPSetDescription(edm::ParameterSetDescription& desc) {
